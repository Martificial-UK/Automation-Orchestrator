...
class AutomationOrchestratorUser(HttpUser):
    ...
    def get_workflows(self):
        """Get all workflows - 10% of traffic (most common)"""
        headers = self.get_headers()
        self.client.get("/api/workflows", headers=headers, name="/api/workflows")
    
    @task(5)
    def get_workflow_status(self):
        """Get specific workflow status - 5% of traffic"""
        headers = self.get_headers()
        self.client.get("/api/workflows/1/status", headers=headers, name="/api/workflows/{id}/status")
    
    @task(3)
    def create_workflow(self):
        """Create new workflow - 3% of traffic"""
        headers = self.get_headers()
        headers["Content-Type"] = "application/json"
        
        workflow_data = {
            "name": f"Stress Test Workflow {int(time.time())}",
            "description": "Auto-generated workflow for stress testing",
            "triggers": ["manual"],
            "actions": [
                {
                    "type": "email",
